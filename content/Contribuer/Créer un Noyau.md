>[!tip]
>Les normes et les conseils que vous trouverez ici découlent de la [[Philosophie|Philosophie de Mind Over Matter]]. N'hésitez pas à la consulter avant de poursuivre.

Comme son nom l'indique, le Noayu est au coeur du système de règle. Il contient au moins 3 parties obligatoires :
- **L'Intention**, dans laquelle l'auteur décrit les raisons qui l'ont poussé à concevoir ce Noyau, et le type d'expérience pour laquelle il est conçu.
- **Le Système de Résolution d'Action**, dans lequel l'auteur décrit comment les dès valident ou invalident la narration proposée par les Joueuses et la Conteuse. Il explique également l'esprit de ces règles, et comment elles doivent amener le type d'expérience souhaitée dans l'Intention.
- **La Création de Personnage**, dans laquelle l'auteur décrit décrit la procédure de création des Personnages des Joueuses, et en quoi ces Personnages vont influer sur les modalités de résolution de l'action.

>[!important]
>**Il n'y a aucune contrainte de taille ou de complexitée.** Le système de règles peux tenir sur un post-it, ou au contraire détailler chaque cas de figure possible. Seul compte le lien entre le Système proposé, et l'Intention qui a suscité la création du Noyau.

La séparation entre ce qui tient du Noyau et ce qui tient d'un [[Créer un Module|Module facultatif]] est également au libre choix de l'auteur.  En fonction de l'Intention, certaines situations seront décrites avec précision dans le Noyau, ou au contraire laissées à la libre interprétation d'un Module facultatif.

Une fois votre Noyau écrit et testé, vous êtes encouragé à le [[Comment contribuer|partager à la communauté]].

TODO : intégrer des conseils de FALC

